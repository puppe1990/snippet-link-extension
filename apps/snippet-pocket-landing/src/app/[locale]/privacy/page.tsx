...
export async function generateMetadata({ params }: PrivacyPageProps): Promise<Metadata> {
  ...
  return getLocaleMetadata(resolveLocale(localeParam), "/privacy");
}
...
export default async function PrivacyPage({ params }: PrivacyPageProps) {
  ...
  return (
    <main className="mx-auto max-w-4xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
      <h1 className="text-3xl font-black text-slate-900">{copy.legal.privacyTitle}</h1>
      <p className="mt-2 text-sm text-slate-500">{copy.legal.updatedAt}</p>

      <div className="mt-6 space-y-5">
        {copy.legal.privacySections.map((section) => (
